...
int main(int argc, char* argv[])
{
    ...
    case 'F'://forward list
    {
        ...
        else if (strcmp(&argv[2][1], "ADDBEFORE") == 0)
        {
            if (argc != 6)
            {
                printHelp();
            ...
            string keystr = argv[3];
            string targetstr = argv[4];
            string occurstr = argv[5];

            if (!isNumber(occurstr))
            {
                cout << "Номер вхождения - не число\n";
                return 1;
            }

            fl->addBefore(targetstr, keystr, stoi(occurstr));
            ofstream ofs(file);
            writeForwardList(ofs, fl);
            ofs.close();
        }
        // Удалить ДО элемента
        else if (strcmp(&argv[2][1], "REMOVEBEFORE") == 0)
        {
            if (argc != 5)
            {
                printHelp();
            ...
            string targetstr = argv[3];
            string occurstr = argv[4];
            ...
            fl->removeBefore(targetstr, stoi(occurstr));
            ofstream ofs(file);
            writeForwardList(ofs, fl);
            ofs.close();
        }
    ...
    case 'L'://list
    {
        ...
        else if (strcmp(&argv[2][1], "ADDAFTER") == 0)
        {
            if (argc != 6)
            {
                printHelp();
            ...
            fl->addAfter(targetstr, keystr, stoi(occurstr));
            ofstream ofs(file);
            writeList(ofs, fl);
            ofs.close();
            }
            // Удалить ПОСЛЕ элемента
        else if (strcmp(&argv[2][1], "REMOVEAFTER") == 0)
        {
            if (argc != 5)
            {
                printHelp();
            ...
            fl->removeAfter(targetstr, stoi(occurstr));
            ofstream ofs(file);
            writeList(ofs, fl);
            ofs.close();
            }
    ...
    case 'T'://tree (BST)
    {
        ...
        if (readTree(ifs, t))
        {
            cout << "В файле не дерево или некорректный формат\n";
            return 1;
        }
        ifs.close();
        ...
        else if (strcmp(&argv[2][1], "FIND") == 0)
        {
            ...
            TreeNode* node = t->search(stoi(keystr));
            if (node != nullptr)
                cout << "Элемент " << keystr << " найден\n";
            else
                cout << "Элемент " << keystr << " не найден\n";
        }
        else if (strcmp(&argv[2][1], "REMOVE") == 0)
        {
            if (argc != 4)
            ...
            if (node != nullptr)
            {
                t->remove(stoi(keystr));
                ofstream ofs(file);
                writeTree(ofs, t);
                ofs.close();
                cout << "Элемент " << keystr << " удален\n";
            }
            else
            {
                cout << "Элемент " << keystr << " не найден\n";
        ...
        {
            cout << "Неизвестная операция для дерева\n";
            return 1;
        }
        break;
    }
    default:
    {
        cout << "Неизвестная структура данных\n";
        return 1;
    }
    }
