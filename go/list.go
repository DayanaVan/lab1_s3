...
func (this *List) addBefore(key string, index int) {
    if index == 0 {
        this.addHead(key)
        return
    }
...
func (this *List) addAfter(key string, index int) {
    node := this.head
    for i := 0; i < index; i++ {
        if node == nil {
            return
...
func (this *List) removeBefore(index int) {
    if index <= 1 {
        return
    }
...
func (this *List) removeAfter(index int) {
    node := this.head
    for i := 0; i < index; i++ {
        if node == nil {
            return
